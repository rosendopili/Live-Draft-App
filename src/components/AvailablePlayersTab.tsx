...
import { Search, Sparkles, UserPlus, RefreshCw, AlertCircle, TrendingUp, Info } from 'lucide-react';
...
export const AvailablePlayersTab: React.FC<AvailablePlayersTabProps> = ({
  ...
  const handleFetchAiRecommendation = async () => {
    ...
    try {
      ...
      const prompt = `Expert advice for ${myTeamName} (Drafting at slot ${myCol}). 
      Format: ${settings.scoring_format}. 
      My Roster: ${myRoster.join(', ') || 'Empty'}. 
      Top Available: ${topAvail.join(', ')}. 
      It is currently MY TURN to pick. Give 3-4 strategic bullet points on who I should pick and why.`;
        ...
        <div className="flex justify-between items-center">
          <div className="space-y-1">
            <h2 className="text-xl font-bold flex items-center gap-2">
              Advisor 
              {isMyTurn && <span className="text-[10px] bg-emerald-500 text-black px-2 py-0.5 rounded-full font-black animate-pulse">YOUR TURN</span>}
            </h2>
            <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex gap-3">
              <span>Current Pick: #{currentPick}</span>
              <span>Drafting for: {settings.team_names[currentCol] || `Team ${currentCol}`}</span>
            </div>
          </div>
          ...
          {isMyTurn ? (
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-800/50 border border-slate-700 rounded-xl text-[10px] font-bold text-slate-400">
              <Info className="w-3.5 h-3.5" />
              WAITING FOR YOUR TURN (SLOT {myCol})
            </div>
          )}
        </div>
        
        {aiAnalysis && <div className="bg-emerald-950/20 border border-emerald-500/20 rounded-xl p-5 text-sm leading-relaxed whitespace-pre-line text-emerald-100">{aiAnalysis}</div>}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div className="flex flex-col lg:flex-row gap-4 mb-8">
          <div className="flex flex-wrap gap-1.5 flex-1">
            {POSITIONS.map(pos => (
              <button key={pos} onClick={() => setSelectedPos(pos)} className={`px-4 py-2 rounded-xl text-xs font-black border transition-all ${selectedPos === pos ? 'bg-emerald-600 border-emerald-500' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}>{pos}</button>
            ))}
          </div>
          ...
          {availablePlayers.slice(0, 100).map(p => (
            <div key={p.id} className="p-4 bg-slate-950/40 border border-slate-800 rounded-2xl flex items-center justify-between group hover:border-emerald-500/30 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div><div className="font-bold text-sm text-slate-100">{p.name}</div><div className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">{p.nflTeam} • Rank #{p.rank}</div></div>
              </div>
